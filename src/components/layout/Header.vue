...
const menuItems = computed(() => [
  ...
  { label: t('header.menu.equipment'), path: "/equipo" },
  { label: t('header.menu.routes'), path: "/rutas" },
  { label: t('header.menu.community'), path: "/comunidad" },
]);
...
<template>
  <nav class="bg-emerald-600 shadow-lg sticky top-0 z-50 border-b border-emerald-500/30">
    <div class="relative z-20 bg-linear-to-r from-emerald-600 to-teal-700 backdrop-blur-sm px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto flex justify-between items-center h-16 lg:h-20">
        ...
        <div class="flex items-center gap-8">
          <ul class="hidden lg:flex place-content-around gap-8 list-none m-0 p-0 items-center">
    ...
    >
      <div v-if="isMobileMenuOpen" class="absolute top-full left-0 w-full z-10 lg:hidden bg-emerald-800/80 backdrop-blur-md border-t border-emerald-500/50 px-4 py-6 shadow-2xl">
        <div class="flex flex-col gap-6">
          ...
          <div class="px-6 pt-4 border-t border-white/10 flex justify-center">
            <LanguageSwitcher />
          </div>
        </div>
      </div>
